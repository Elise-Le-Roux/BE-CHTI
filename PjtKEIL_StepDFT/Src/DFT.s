...
; 	int Im = 0, Re = 0;
; 	for (int n = 0; n < 64, n++) {
; 		Im += Signal64ech[n] * sin(2PI*k*n/M);
; 		Re += Signal64ech[n] * cos(2PI*k*n/M);
; 	}
; 	return Im^2 + Re^2;
; }
...
DFT_ModuleAuCarre proc
		push{r4-r7}
		; r0 = adresse de x(0) & r1 = k
		mov r2, #0 ; index n
		mov r7, #0 ; partie réelle
		mov r12, #0 ; partie imaginaire

For     ; boucle for(int n = 0; n < 64, n++)
		ldrsh r10, [r0, r2, lsl #1] ;r3 = x(n) 
		ldr r4, =TabCos ; 
		ldr r8, =TabSin ; 
		mul r5, r1, r2 ; r5 = k * n
		and r5, #0x0000003F ; r5 = p = k*n mod 64
		ldrsh r6, [r4, r5, lsl #1] ; r6 = TabCos(p)
		ldrsh r9, [r8, r5, lsl #1] ; r9 = TabSin(p)
		mul r3, r10, r6 ; r3 = x(n)*cos(2PI*k*n/M)
		mul r11, r10, r9 ; r11 = x(n)*sin(2PI*k*n/M)
		add r7, r3 ; partie réelle
		add r12, r11 ; partie imaginaire
		add r2, #1
		cmp r2, #64
		bne For
		
		smull r1, r0, r7, r7   ; Re^2
